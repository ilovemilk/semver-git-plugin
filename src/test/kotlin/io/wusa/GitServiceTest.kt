...
class GitServiceTest {
    ...
    fun `get describe for tagged semver commit`() {
        ...
        Assertions.assertEquals(Version(1, 0, 0, "", "", null, project), GitService.describe("0.1.0","none", project))
    }
    ...
    fun `get describe for tagged non-semver commit`() {
        ...
        Assertions.assertThrows(IllegalArgumentException::class.java) {
            GitService.describe("0.1.0","none", project)
        }
    }
    ...
    fun `get describe for not tagged commit`() {
        ...
        Assertions.assertEquals(Version(1, 0, 0, "", "", Suffix(0, "1234567", true), project), GitService.describe("0.1.0", "none", project))
    }

    @Test
    fun `get describe without tags`() {
        every { GitCommandRunner.execute(projectDir = any(), args = any()) } throws GitException("error")
        Assertions.assertEquals(Version(0, 1, 0, "", "", Suffix(0, "", false),project), GitService.describe("0.1.0", "none", project))
    }
}